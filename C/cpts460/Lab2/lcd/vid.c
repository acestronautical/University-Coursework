...
int fbuf_init() {
   ...
   *(volatile unsigned int *)(0x10120010) = (1*1024*1024);  *(volatile unsigned
   *t *)(0x10120018) = 0x82B;
   ***************************************************************/
...
int clrcursor() { unkpchar(cursor, row, col); }

int putcursor(unsigned char c) { kpchar(c, row, col); }
...
int show_bmp(char *p, int startRow, int startCol) {
  ...
  for (i = startRow; i < (h / 2) + startRow; i += 1) {
    pp = p;
    for (j = startCol; j < (w / 2) + startCol; j += 1) {
      b = *pp;
      g = *(pp + 1);
      r = *(pp + 2);
      pixel = (b << 16) + (g << 8) + r;
